...
#     partition_GNN = nx_comm.louvain_communities(GNN_sim, weight='weight', seed=42)
    ...
    modularity_GNN = 0#round(nx.algorithms.community.quality.modularity(GNN_sim, partition_GNN, weight='weight'),3) # modularità su tutto il grafo
...
#             n_overlap += 1
    ...
    degree_k_tau, in_degree_k_tau = all_K_tau(G0, GNN_sim)#all_K_tau(G0, GNN_sim, GNN_dist)
#     print(degree_k_tau)
    #average clustering
    
    avg_cluster = 0#nx.average_clustering(GNN_sim, weight = 'weight')
    
    return modularity_GNN, degree_k_tau, in_degree_k_tau, assortativity_GNN, max_jaccard, avg_cluster
...
def K_tau(dict1,dict2):
    ...
    x1 = list(sorted_dict1.keys())[:th]
    x2 = list(sorted_dict2.keys())[:th]
...
def all_K_tau(G0, G_sim):
    ...
    return K_tau(degree_G0, degree_Gsim), 0
