...
def train_step(G0, G_train,model,optimizer,labels,list_mat,idx_train):
    ...
    structure_penalty =   mod_w*(out_degree_k_tau) #+ out_degree_k_tau + in_degree_k_tau + assortativity_GNN + jacc
    ###########################
...
def validate_step(G0, G_val, model,labels,list_mat,idx_val):
    ...
    with torch.no_grad():
        ...
        structure_penalty =   mod_w*( avg_cluster)# + out_degree_k_tau + in_degree_k_tau + assortativity_GNN + jacc
