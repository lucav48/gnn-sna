...
def calculate_metrics(G,G_sub, embedding, idx, lambdas):
...
#     partition_GNN = nx_comm.greedy_modularity_communities(GNN_sim, weight='weight')
    ...
    if 5 in lambdas:
        out_degree_k_tau, in_degree_k_tau = all_K_tau(G0, GNN_sim)
    else:
        out_degree_k_tau, in_degree_k_tau = 0, 0

    #average clustering  
    if 4 in lambdas:
        avg_cluster = nx.average_clustering(GNN_sim, weight = 'weight')
    else:
        avg_cluster = 0
    return modularity_GNN, out_degree_k_tau, in_degree_k_tau, 0, max_jaccard, avg_cluster
