...
def preprocess_features(features):
    ...
    r_inv = np.float_power(rowsum, -1).flatten() #np.power(rowsum, -1).flatten()
...
def full_load_data(dataset_name, splits_file_path=None):
    ...
    return adj,adj_i, features, labels, train_mask, val_mask, test_mask, num_features, num_labels, g

'''def full_load_data_custom(dataset_name, splits_file_path=None):
    if dataset_name in {'cora', 'citeseer', 'pubmed'}:
        adj, features, labels, _, _, _ = full_load_citation(dataset_name)
        labels = np.argmax(labels, axis=-1)
        features = features.todense()
